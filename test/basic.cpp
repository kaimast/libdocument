...
TEST(Basic, array_nested_multi_append) {
    ...
    EXPECT_EQ(
        doc.str(),
        "{\"b\":\"xyz\",\"a\":{\"foo\":[4,3,2,23,23,23,23,23]},\"bar\":1337}");
}
...
TEST(Basic, diffs) {
    ...
    EXPECT_EQ(diffs.begin()->as_document(),
              json::Document(
                  "{\"type\":\"modified\",\"path\":\"a\",\"new_value\":2}"));
}
...
TEST(Basic, diffs_str) {
    ...
    EXPECT_EQ(diffs.size(), 1U);
    EXPECT_EQ(
        diffs.begin()->as_document(),
        json::Document(
            "{\"type\":\"modified\",\"path\":\"a\",\"new_value\":\"we go\"}"));
}
...
TEST(Basic, diff_deleted) {
    ...
    EXPECT_EQ(diffs.size(), 1U);
    EXPECT_EQ(diffs.begin()->as_document(),
              json::Document("{\"type\":\"deleted\",\"path\":\"a\"}"));
}

...
class DocumentMerger : public DocumentTraversal {
  public:
    DocumentMerger(bitstream &doc, std::string full_path,
                   const bitstream &other)
  ...
  private:
    void insert_into_map(const std::string &key, const bitstream &data,
                         uint32_t start) {
        ...
        uint32_t byte_size =
            ...
            sizeof(byte_size); // this is the start of the map not the element
    ...
    bool is_array_insertion(const std::string &str) { return str == "+"; }

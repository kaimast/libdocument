...
class DocumentDiffs : public DocumentTraversal {
  public:
    ...
    void create_diffs(Diffs &diffs) { parse_next(diffs, false); }
  ...
  private:
    void parse_next(Diffs &diffs, bool inside_diff) {
        ...
        if (type1 != type2) {
            ...
            diffs.emplace_back(Diff(DiffType::Modified, path_string(path1),
                                    &view2.data()[start], end - start));
        } else {
            switch (type1) {
            case ObjectType::String: {
                ...
                if (!inside_diff && str1 != str2) {
                    diffs.emplace_back(Diff(DiffType::Modified,
                                            path_string(path1),
                                            &view2.data()[start], end - start));
                }
                break;
        ...
        while (i < size1 || j < size2) {
            ...
            if (key1 == key2) {
                ...
                if (i < size1) {
                    ...
                    diffs.emplace_back(Diff(DiffType::Deleted,
                                            path_string(path1),
                                            &view1.data()[start], end - start));
                }
                ...
                if (j < size2) {
                    ...
                    diffs.emplace_back(Diff(DiffType::Added, path_string(path2),
                                            &view2.data()[start], end - start));
                }
            }
...
class DocumentAdd : public DocumentTraversal {
  public:
    DocumentAdd(bitstream &data, const std::string &path,
    ...
    void do_add() {
        ...
        if (t != ObjectType::Integer && t != ObjectType::Float &&
            t != ObjectType::String) {
        ...
        if (on_target) {
            if (type == ObjectType::Integer) {
                static_assert(sizeof(json::integer_t) == sizeof(json::float_t),
                              "integer and float must have same size");
                ...
                if (m_value.get_type() == ObjectType::Integer) {
                    ...
                    m_view.move_by(-1 *
                                   static_cast<int32_t>(sizeof(ObjectType) +
                                                        sizeof(value)));
